...
window.onYouTubeIframeAPIReady = function() {
    document.querySelectorAll('.yt-container').forEach((container, idx) => {
        ...
        if (hoverTarget && hoverTarget.dataset.behavior === 'hover' && !isTouch) {
            hoverTarget.onmouseenter = () => player.playVideo();
            hoverTarget.onmouseleave = () => { if (!document.fullscreenElement && !document.webkitFullscreenElement) player.pauseVideo(); };
        }
    });
};
